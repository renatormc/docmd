...
class UndefinedSilently(Undefined):
    ...
    __call__ = return_new  # type: ignore
...
def pad_string(value: str, length=5) -> str:
    value = str(value)
...
class Renderer(object):
    ...
    def __init__(self, environment: Environment | None = None, **kwargs) -> None:
        """
        ...
        self.template_images: dict[str, dict[str, Any]] = {}
        self.subdoc_counter = 1
        self.replace_table_names: list[str] = []
    ...
    def finalize_value(self, eval_ctx, value: Any, *args):
    ...
    def media_loader(self, callback: Callable) -> Callable:
    ...
    def _unpack_template(self, template: str) -> dict[str, bytes]:
    ...
    def _pack_document(self, files: dict[str, bytes]) -> io.BytesIO:
    ...
    @staticmethod
    def _inc_node_tags_count(node: 'Node | None', is_block: bool = False) -> None:
        ...
        node.field_count += 1  # type: ignore
        if is_block:
            node.block_count += 1  # type: ignore
        else:
            node.var_count += 1  # type: ignore

        Renderer._inc_node_tags_count(node.parentNode, is_block)

    def _compile_tags_expressions(self) -> None:
    ...
    def _compile_escape_expressions(self) -> None:
        ...
        for key, value in unescape_rules.items():
            ...
            self.escape_map[pattern] = r'\1{0}\4'.format(value)

    def _is_jinja_tag(self, tag: str) -> bool:
        """
    ...
    def _is_block_tag(self, tag: str) -> bool:
        """
            Returns True is tag (str) is a jinja flow control tag.
        """
        return len(self.block_pattern.findall(tag)) > 0

    def _tags_in_document(self, document: Document) -> Iterable[Element]:
        """
    ...
    def _census_tags(self, document: Document) -> None:
        """
    ...
    def _prepare_document_tags(self, document: Document) -> None:
    ...
    def _unescape_links(self, xml_text: str) -> str:
    ...
    @staticmethod
    def get_escaped_var_value(value: str) -> str:
        """
    ...
    def add_media_to_archive(self, media: io.BytesIO, mime: str, name='') -> str:
        """
        ...
        if hasattr(media, 'name') and not name:
            split_result = path.splitext(media.name)
            name = split_result[0]
    ...
    def fs_loader(self, media: io.BufferedReader | str, *args, **kwargs) -> tuple[io.BufferedReader, str | None] | None:
        """Loads a file from the file system.
        :param media: A file object or a relative or absolute path of a file.
        :type media: unicode
        """
        if hasattr(media, 'seek') and hasattr(media, 'read'):
            return (media, 'image/jpeg')  # type: ignore
    ...
    def replace_images(self, xml_document):
        ...
        for frame in frames:
            ...
            frame_attrs = dict()
            for i in range(frame.attributes.length):
            ...
            image_attrs = dict()
            for i in range(image_node.attributes.length):
    ...
    def _render_xml(self, xml_document: Document, **kwargs) -> Document:
        ...
        try:
            ...
            xml_source_encoded = xml_source.encode('ascii', 'xmlcharrefreplace')
            # self.xml_saver.save_xml(xml_source_encoded.decode('utf-8'))
            jinja_template = self.environment.from_string(
                self._unescape_entities(xml_source_encoded.decode('utf-8'))
            )
    ...
    def set_docmaker_id(self, xmldoc: Document, id: str) -> Document:
        for el in xmldoc.getElementsByTagName('meta:user-defined'):
            if el.getAttribute('meta:name') == "DocmakerID":
                child = el.firstChild
                if child is None:
                    child = xmldoc.createTextNode(id)
                    ...
                    if hasattr(child, "nodeValue"):
                        ...
                        el.appendChild(xmldoc.createTextNode(id))
                return xmldoc
        new_el = xmldoc.createElement('meta:user-defined')
    ...
    def _parent_of_type(self, node: Element, of_type) -> Element | None:
    ...
    def create_text_span_node(self, xml_document: Document, content: str) -> Element:
    ...
    def get_style_by_name(self, style_name: str) -> Element | None:
        """
    ...
    def insert_style_in_content(self, style_name: str, attributes: dict[str, str] | None = None,
                                **style_properties) -> Element:
        """
    ...
    def markdown_filter(self, markdown_text: str) -> Markup:
        """
    ...
    def image_filter(self, value, *args, **kwargs) -> str:
    ...
    # def image_filter_w(self, value, *args, **kwargs) -> str:
    #     return self.image_filter(value, *args, **kwargs, fixed_side="w")

    # def image_filter_h(self, value, *args, **kwargs) -> str:
    #     return self.image_filter(value, *args, **kwargs, fixed_side="h")

    def subdoc_function(self, template: str, **kwargs) -> Markup:
        files = self._unpack_template(template)
        xml_content = parseString(files['content.xml'])
        xml_styles = parseString(files['styles.xml'])
        rendered_content = self._render_xml(xml_content, **kwargs)
        main_doc_styles = StylesManager(self.content, self.styles)
        sub_doc_styles = StylesManager(rendered_content, xml_styles)
        get_copy_content(main_doc_styles, sub_doc_styles, prefix=f"sub{self.subdoc_counter}")

        styles = self.content.getElementsByTagName('office:automatic-styles')[0]
        for style in main_doc_styles.automatic_styles_for_addition:
            styles.appendChild(style)

        styles = self.styles.getElementsByTagName('office:styles')[0]
        for style in main_doc_styles.default_styles_for_addition:
            styles.appendChild(style)

        self.subdoc_counter += 1
        return Markup(''.join(node.toxml() for node in sub_doc_styles.content_for_addition))

    def table_function(self, table_name: str) -> Markup:
        self.replace_table_names.append(table_name)
        return Markup(f"<replace-table name='{table_name}'/>")

    def replace_tables(self, xml_document: Document) -> None:
        markers = list(xml_document.getElementsByTagName('replace-table'))
        tables = list(xml_document.getElementsByTagName('table:table'))

        for table_name in self.replace_table_names:
            marker = next((node for node in markers if node.getAttribute('name') == table_name), None)
            table = next((node for node in tables if node.getAttribute('table:name') == table_name), None)
            if not table:
                raise SecretaryError(f"Table with name '{table_name}' not found in template")
            paragraph = self._parent_of_type(marker, 'text:p') if marker is not None else None

            if marker is None or paragraph is None or paragraph.parentNode is None or table is None:
                continue

            if table.parentNode is not None:
